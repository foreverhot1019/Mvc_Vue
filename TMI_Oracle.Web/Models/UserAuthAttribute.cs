        ...
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            ...
            if (IAuthenManager.User != null)
            {
                if (IAuthenManager.User.Identity.IsAuthenticated)
                {
                    ...
                    List<MenuItem> ViewMenu = (List<MenuItem>)HttpContext.Current.Session["Login_MenuItem"] ?? null;
                    List<MenuItem> MinSubMenus = new List<MenuItem>();//菜单权限 一对多时（菜单内跳转到子菜单）
                    if (ViewMenu != null && ViewMenu.Any())
                    {
                        ...
                        {
                            MinSubMenus = ViewMenu.Where(x => !x.SubMenus.Any() && x.Controller.ToLower().StartsWith(controllerName.ToLower())).ToList();
                            var WhereMinSubMenus = MinSubMenus.Where(x=>x.Action.ToLower().StartsWith(actionName.ToLower()));
                            if (WhereMinSubMenus.Any())
                                MinSubMenus = WhereMinSubMenus.ToList();
                        }
                        #region 设置当前 点击菜单 前台选中

                        var OMenu = MinSubMenus.Where(x => x.IsEnabled && !x.SubMenus.Any()).OrderByDescending(x => x.Id).FirstOrDefault();
                        if (OMenu != null && OMenu.Id > 0)
                        {
                            OHttpContext.Response.Cookies.Set(new HttpCookie("SeltMenuId", OMenu.Id.ToString()));
                        }
                    ...
                    #region 无需验证权限的页面配置(home页，404，500等)
                    ...
                    {
                        OHttpContext.Response.Cookies.Set(new HttpCookie("SeltMenuId", null));
                        return;
                    }
                    ...
                    if (ViewMenu != null && ViewMenu.Any())
                    {
                        if (MinSubMenus.Any())
                        {
                            ...
                            {
                                //action 不是 创建，编辑，删除，导入，导出，审批的一种
                                //那么默认 查看，具体按钮权限 在页面上控制
                                if (actionName.ToLower() != "view")
                                {
                                    //View查看权限 MenuAction是必须的
