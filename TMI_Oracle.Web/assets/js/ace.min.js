...
jQuery(function (a)
{
    try {
        var demo = ace.demo;
        if (!ObjectIsEmpty(demo)){
            var init = demo.init;
            if(typeof (init) === 'function')
                ace.demo.init();
        }
        ////记住选择的样式
        //var aceSkin = ace.storage.get('ace_skin');
        //if (!(typeof (aceSkin) === undefined || aceSkin == null || aceSkin==''|| aceSkin == 'no-skin')){
        //    var $SkinSelter = $("#skin-colorpicker");
        //    var $option = $SkinSelter.find("option[data-skin='"+aceSkin+"']:first");
        //    if ($option.length > 0) {
        //        var color = $option.val();
        //        $SkinSelter.val(color).ace_colorpicker('pick', color);
        //        $SkinSelter.trigger('change');
        //    }
        //}
        ////菜单高亮
        //var acehighlight = ace.storage.get('ace_state_id-ace-settings-highlight');
        //if (!(typeof (acehighlight) === undefined || acehighlight == null || acehighlight == '')) {
        //    var d = JSON.parse(acehighlight)
        //    var checked = d.checked;
        //    if (typeof (checked) === 'undefined' || checked == null || typeof (checked) === 'boolean') {
        //        ace.settingFunction.alt_active_item(null, checked)
        //    }
        //}
    } catch (b) {
        console.log(b);
...
function (a, b)
{
    ...
    ace.demo.functions.aceSkin = function ()
    {
        var aceSkin = ace.storage.get('ace_skin');
        if (!(typeof (aceSkin) === undefined || aceSkin == null || aceSkin==''|| aceSkin == 'no-skin')){
            var $SkinSelter = $("#skin-colorpicker");
    ...
    ace.demo.functions.aceHighlight = function ()
    {
        var acehighlight = ace.storage.get('ace_state_id-ace-settings-highlight');
        if (!(typeof (acehighlight) === undefined || acehighlight == null || acehighlight == '')) {
            var d = JSON.parse(acehighlight)
...
function (a, b)
{
    function c(b, c)
    {
        ...
        this.$sidebar.on(ace.click_event + ".ace.submenu", ".nav-list",//点击菜单
        function (b)
        {
            var c = this,
...
function (a, b)
{
    ...
    {
        //2019-02-27 增加
        ace.settingFunction.alt_active_item(null, this.checked)
    ...
    function (b, c, d, e, f)
    {
        ...
        switch (c) {
            ...
            case "main_container_fixed":
                ...
                break;
            case "alt_active_item":
                //2019-02-27 增加
                g = "ace-settings-highlight";
                break;
        }
    ...
    ace.settingFunction = {
        ...
        alt_active_item: function (b, c, d, e)
        {
            //2019-02-27 增加
            //c ? a("#sidebar .nav-list > li").addClass("highlight") : a("#sidebar .nav-list > li").removeClass("highlight"),
            //ace.vars.old_ie && ace.helper.redraw(a("#sidebar")[0])

            if (ace.vars.very_old_ie) return !1;
            var b = b || "#sidebar .nav-list li";//自动设置 样式 只能一个
            if ("string" == typeof b && (b = a(b).get(0)), !b) return !1;
            var f, c = c || !1,
            d = "undefined" != typeof d ? d : !0;
            return a(document).trigger(f = a.Event("presettings.ace"), ["alt_active_item", c, b, d]),
            f.isDefaultPrevented() ? !1 : (c ? a("#sidebar .nav-list li").addClass("highlight") : a("#sidebar .nav-list li").removeClass("highlight"), d && ace.settings.saveState(b, "class", "alt_active_item", c), ace.vars.old_ie && ace.helper.redraw(a("#sidebar")[0]), void a(document).trigger("settings.ace", ["alt_active_item", c, b, d]))
        }
    }
}(jQuery),
function (a, b)
