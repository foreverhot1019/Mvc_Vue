...
{
    public class FinanceMoneyQuery : QueryObject<FinanceMoney>
    {
        public FinanceMoneyQuery WithAnySearch(string search)
        {
        ...
        public FinanceMoneyQuery WithPopupSearch(string search, string para = "")
        {
            if (!string.IsNullOrEmpty(search))
                And(x =>
                x.Id.ToString().Contains(search) ||
        ...
        public FinanceMoneyQuery Withfilter(IEnumerable<filterRule> filters)
        {
            if (filters != null)
            {
