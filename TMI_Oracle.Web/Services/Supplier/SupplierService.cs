...
namespace TMI.Web.Services
{
    public class SupplierService : Service<Supplier>, ISupplierService
    {
        ...
        {
            var mapping = _mappingservice.Queryable().Where(x => x.EntitySetName == "Supplier").ToList();
            foreach (DataRow row in datatable.Rows)
            {
                ...
                foreach (var field in mapping)
                {
                    ...
                    if (contation && row[field.SourceFieldName] != DBNull.Value)
                    {
                        ...
                        PropertyInfo propertyInfo = suppliertype.GetProperty(field.FieldName);
        ...
        public Stream ExportExcel(string filterRules = "", string sort = "Id", string order = "asc")
        {
            ...
            var supplier = this.Query(new SupplierQuery().Withfilter(filters)).OrderBy(n => n.OrderBy(sort, order)).Select().ToList();
