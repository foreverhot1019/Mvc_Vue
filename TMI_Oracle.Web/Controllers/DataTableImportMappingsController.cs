...
namespace TMI.Web.Controllers
{
    public class DataTableImportMappingsController : Controller
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public ActionResult GetAllEntites(string Title="")
        {
            var ArrTypes = (List<Type>)CacheHelper.Get_SetCache(Common.CacheNameS.AllEntityAssembly);
            ArrTypes = ArrTypes.Where(type => typeof(Repository.Pattern.Ef6.Entity).IsAssignableFrom(type)).ToList();
            var rows = ArrTypes.Select(x => new { Name = x.Name, Value = x.Name });
            return Json(rows, JsonRequestBehavior.AllowGet);

            //var list = _dataTableImportMappingService.GetAssemblyEntities();
            //var rows = list.Select(x => new { Name = x.FieldTypeName, Value = x.EntitySetName }).Distinct();
            //return Json(rows, JsonRequestBehavior.AllowGet);
        }
