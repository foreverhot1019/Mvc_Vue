...
namespace TMI.Web.Controllers
{
    ...
    public class RoleMenusController : Controller
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetRolesVue()
        {
            var roles = _roleManager.Roles;
            var roleview = new List<RoleView>();
            ...
            return Json(roleview, JsonRequestBehavior.AllowGet);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult GetMenuListVue()
        {
            List<MenuAction> MenuActions = new List<MenuAction>();
            MenuActions = (List<MenuAction>)CacheHelper.Get_SetCache(Common.CacheNameS.MenuAction);
            var MenuActionRep = _unitOfWork.Repository<MenuAction>();
            var menus = _menuItemService.Queryable().Include(x => x.SubMenus).ToList();//.Where(x => x.IsEnabled);
            var totalCount = menus.Count();
            var func = new Func<MenuItem, int>(Menu =>
            {
                int level = 1;
                while (Menu != null && Menu.Id > 0 && Menu.ParentId.HasValue && Menu.ParentId > 0)
                {
                    level++;
                    Menu = menus.Where(x => x.Id == Menu.ParentId).FirstOrDefault();
                }
                return level;
            });
            var datarows = menus.OrderBy(x => x.Code).Select(x => new
            {
                ...
                x.IsEnabled,
                Level = func(x),
                Children = (x.SubMenus == null || !x.SubMenus.Any()) ? null : x.SubMenus.Select(n => n.Id),
                __Expended = true,//展开/折叠
                __ExpendAror = true,//展开/折叠 箭头
            });
            //var ObjArrMenu = datarows.Select(x => new
            //{
            //    MenuId = x.Id,
            //    Menu = new
            //    {
        ...
        /// <param name="roleName"></param>
        /// <returns></returns>
        public ActionResult GetMenusVue(string RoleId)
        {
            var rolemenus = _roleMenuService.Queryable().Where(x => x.RoleId == RoleId);
            ...
            StringBuilder SQLStrBCas = new StringBuilder();
            StringBuilder SQLStrBC = new StringBuilder();
            List<string> fieldList = new List<string>() { "Id", "RoleId", "MenuId", "RoleName", "IsEnable" }; //From config or db
            if (MenuActions.Any())
            {
                foreach (var item in MenuActions)
                {
            ...
            List<dynamic> ArrObj = new List<dynamic>();

            string SQLStr = "select m.RoleId,m.MenuId,m.RoleName " + SQLStrBC.ToString() + " from ( select RoleId,MenuId" + SQLStrBMax.ToString() + " from ( select a.Id,a.MenuId,a.RoleId" + SQLStrBCas.ToString() + " from RoleMenuActions a ) b group by RoleId,MenuId ) c left join RoleMenus m on m.RoleId = c.RoleId and m.MenuId = c.MenuId where m.RoleId = @V_RoleId";
            SqlParameter[] param = new SqlParameter[]{
                 new SqlParameter ("V_RoleId", RoleId )
            };
            DataSet ds = SQLDALHelper.SQLHelper.GetDataSet(CommandType.Text, SQLStr.ToUpper(), param);
            if (ds != null)
            {
                if (ds.Tables.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        dynamic Topdobj = new System.Dynamic.ExpandoObject();
                        Topdobj.MenuId = dr["MenuId"].ToString();
                        Topdobj.Menu = dobj;
                        ArrObj.Add(Topdobj);
                        //ArrObj.Add(dobj);
                    }
            ...
            var result = new { total = ArrObj.Count, rows = ArrObj };
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult SubmitVue(string JsonStrSeltMenu)
        {
            List<MenuAction> MenuActions = new List<MenuAction>();
            if (HttpContext.Cache[Common.GeCacheEnumByName("MenuAction").ToString()] != null)
            {
            ...
            //Json数据格式 转换成 匿名类
            dynamic dyobj = Newtonsoft.Json.JsonConvert.DeserializeObject(JsonStrSeltMenu);
            System.Reflection.PropertyInfo[] PropertyInfos = typeof(RoleMenuAction).GetProperties(System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.NonPublic);
            foreach (var item in dyobj)
            {
                //获取 dynamic 数据集 
                Dictionary<string, object> dicDynamics = Common.GetDictKeyValueForDynamic(item);
            ...
            if (ArrRleMenAct.Any())
            {
                var ArrChgRoleId = new List<string>();//变更的权限ID
                var RoleMenuActionRep = _unitOfWork.Repository<RoleMenuAction>();
                var RoleMenuRep = _unitOfWork.Repository<RoleMenu>();
                var Roles = RoleManager.Roles.ToList();
                ...
                try
                {
                    #region 更新 被修改权限关联的账户，安全票据

                    var ChgRoleIds = ArrChgRoleId.Distinct();
                    var ChgRoleInUserIds = UserManager.Users.Where(x => x.Roles.Any(n => ChgRoleIds.Contains(n.RoleId))).Select(x => x.Id).ToList();
                    if (ChgRoleInUserIds != null && ChgRoleInUserIds.Any())
                    {
                        foreach (var UserId in ChgRoleInUserIds)
                            //更新安全票据
