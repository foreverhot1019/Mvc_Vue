    ...
    public class WordHelper
    {
                    ...
                    foreach (var bookmark in bookMarks)
                    {
                        ...
                        if (book_mark.Name.IndexOf("CM_Length") > -1 || book_mark.Name.IndexOf("CM_Width") > -1 || book_mark.Name.IndexOf("CM_Height") > -1 || book_mark.Name.IndexOf("CM_Piece") > -1)
                        {
                            foreach (var value in ArrValue)
                            {
                                ...
                                {
                                    var num = book_mark.Name.Substring(9,1).ToString().ToInt32();
                                    if (arrnum > num)
                                    {
                                        var name = book_mark.Name.Substring(0, 9).ToString();
                                        valStr.GetProperties();
                                        var item = temp[num];
                                ...
                                {
                                    var num = book_mark.Name.Substring(8, 1).ToString().ToInt32();
                                    if (arrnum > num)
                                    {
            ...
            where T : class,new()
        {
            ...
            if (wordModelPath == "ExportBilOfLadinPDF")
            {
                ...
                builder.InsertCell();
                setCell(builder, font, "", "TotalPiecesNum", MaxWidth, 8, Math.Round((decimal)TotalPieces, 3).ToString("#0.00"), dictHeaherWidth, g, ivalue);

                builder.InsertCell();
                setCell(builder, font, "", "TotalWeight", MaxWidth, 8, "合计毛重", dictHeaherWidth, g, ivalue);
                builder.InsertCell();
                setCell(builder, font, "", "TotalWeightNum", MaxWidth, 8, Math.Round((decimal)TotalWeight, 3).ToString("#0.00"), dictHeaherWidth, g, ivalue);
                builder.CellFormat.HorizontalMerge = CellMerge.First;
                builder.InsertCell();
                builder.CellFormat.HorizontalMerge = CellMerge.Previous;
                ...
                builder.InsertCell();
                setCell(builder, font, "", "TotalVolumeNum", MaxWidth, 8, Math.Round((decimal)TotalVolume, 3).ToString("#0.000"), dictHeaherWidth, g, ivalue);
                builder.CellFormat.HorizontalMerge = CellMerge.First;
                builder.InsertCell();
                builder.CellFormat.HorizontalMerge = CellMerge.Previous;
            ...
            foreach (var item in ArrFields)
            {
                ...
                dictHeaherWidth.Add(dmic.Name, cellwidth);
            }
            builder.EndRow();
            #endregion
