...
function boolformatter(value, row, index) {
    //console.log('boolformatter', value);
    try {
        if (typeof value === 'undefined')
            return value;
...
function ExtractFlagformatter(value, row, index) {
    try {
        if (typeof value === 'undefined')
            return value;
        if (value == null)
...
//结合数据级进行转换
function formatterByEnumArray(value, row, index, ArrFData,TextField) {
    try {
        if (typeof (TextField) == 'undefined' || TextField == null || TextField == '')
            TextField = "DisplayName";
        if (typeof (ArrFData) == 'undefined' || ArrFData == null || ArrFData.length <= 0) {
            Usedformatter(value, row, index);
        } else {
            if (value == null)
                return value;
            else {
                for (var i in ArrFData) {
                    if (ArrFData[i].Value == value) {
                        return ArrFData[i][TextField];
                    }
                }
                return value;
            }
...
function UsedformatterByArrStatus(value, row, index) {
    return formatterByEnumArray(value, row, index, ArrStatus);
}

function UsedformatterByArrAuditStatus(value, row, index) {
    return formatterByEnumArray(value, row, index, ArrAuditStatus);
}
...
function Flightjoinformatter(value, row, index) {
    try {
        var name = this.field + 'NAME';
        if (row[name])
            return row[name];
...
function Modify_Markformatter(value, row, index) {
    try {
        //0不变/1修改2删除/3新增
        if (typeof value === 'undefined')
            return value;
...
function onBeforeRenderBybyte(target, rows, columns, ControllerName) {
    var ids = [];
    ...
    $.ajax({
        type: 'POST',
        url: '/' + ControllerName + '/GetbyteColumnHtml',//获取数据的函数
        async: false,//true 异步，false 同步
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ ids: ids, bytecolumns: columns }),//查询条件
        error: function () { },//查询失败
        success: function (data) {//查询成功,data为返回的数据
            if (data) {
                //console.log('onBeforeRenderBybyte', data);
                $.each(data, function (x, elem) {
                    $.each(rows, function (i, item) {
                        if (elem[0]["Value"] == item.ID) {
                            for (var i in columns) {
                                for (var x in elem) {
                                    if (elem[x].Key == columns[i])
                                        item[columns[i]] = elem[x].Value;
                                }
                            }
                        }
                    });
    ...
    //        return value;
    //});
    //$.post('/EMS_HEADS/GetbyteColumnHtml', { ids: ids, bytecolumns: ["APPROVAL_DOCUMENTS"] }, function (data, status) {
    //    if (data) {
    //        $.each(data, function (x, elem) {
    //            console.log(elem,elem[0],elem[0]["Value"]);
    //            $.each(rows, function (i, item) {
    //                if (elem[0]["Value"] == item.ID) {
    //                    item.APPROVAL_DOCUMENTS = elem[1]["Value"];
    //                }
    //            });
    //        });
    //    }
...
//总单应付行 业务单号显示 白色
function DzbhFormatter(value, row, index) {
    try {
        if (!ObjectIsEmpty(value)) {
            var IsMBLJS = row.IsMBLJS || row.OPS_BMS_Status //总单应付行 标志
            if (!ObjectIsEmpty(IsMBLJS)) {
                if (ObjectIsTrue(IsMBLJS)) {
                    var ArrStr = [];
                    ArrStr.push("<span style='color:#ffffff;'>");
                    ArrStr.push(value);
                    ArrStr.push("</span>");
                    return ArrStr.join('');
                }
        ...
        console.log('DzbhFormatter-error', e);
        return value;
    }
}
//替换Json特殊字符
...
//是否是True-- int>0= true 'true' = true
//undefined/null 返回 null
function ObjectIsTrue(value) {
    var type = typeof (value);
    if (type === 'undefined')
        return null;
    else if (type === 'boolean') {
        if (value == true) {
            return true; // return inline style;
        }
        if (value == false)
            return false;
    }
        ...
        if (isNaN(value)) {
            var lowCaseVal = value.toLowerCase();
            if (lowCaseVal == 'true')
                return true; // return inline style;
