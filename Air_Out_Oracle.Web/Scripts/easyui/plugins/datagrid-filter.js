(function ($) {
    ...
    $.extend($.fn.datagrid.methods, {
        ...
        loadData: function (jq, data) {
            jq.each(function () {
                $.data(this, 'datagrid').filterSource = null;//{ total: 0, rows: [] };
            });
            ...
            if (opts.pagination) {
                ...
                pager.pagination({
                    onSelectPage: function (pageNum, pageSize) {
                        var boolEndEditing = false;
                        try{
                            if (opts.onNewEndEditing) {
                                boolEndEditing = opts.onNewEndEditing.call(target);
                            } else {
                                //return;
                            }
                        } catch (ex) {
                            boolEndEditing = false;
                        }
                        if (!boolEndEditing) {
                            //return;
                        }
                        ...
                        try{
                            ////console.log('myLoadFilter',boolEndEditing);
                            //var newgetChanges = {};
                            //if (boolEndEditing) {
                            //    var inserted = dg[name]('getChanges', "inserted");
                            //    var deleted = dg[name]('getChanges', "deleted");
                            //    var updated = dg[name]('getChanges', "updated");
                            //    newgetChanges.inserted = $.extend(true, [], inserted);
                            //    newgetChanges.deleted = $.extend(true, [], deleted);
                            //    newgetChanges.updated = $.extend(true, [], updated);
                            //    newgetChanges = $.extend(true, {}, newgetChanges);
                            //    //console.log('myLoadFilter-NewgetChanges', newgetChanges);
                            //}
                        } catch (ex) {
                            boolEndEditing = false;
                        }
                        ...
                        if (boolEndEditing) {
                            //opts.ConcatChanges.call(target, newgetChanges);

                            //console.log(dg, $.data("inserted"));
                            //console.log($.data(dg[0], "datagrid"));
                            //var insertedRows = $.data(dg[0], "datagrid").insertedRows;
                            //var deletedRows = $.data(dg[0], "datagrid").deletedRows;
                            //var updatedRows = $.data(dg[0], "datagrid").updatedRows;
                            //if (newgetChanges.inserted) {
                            //    insertedRows = newgetChanges.inserted;
                            //    //insertedRows.concat(newgetChanges.inserted);
                            //}
                            //if (newgetChanges.deleted) {
                            //    deletedRows = newgetChanges.deleted;
                            //    //deletedRows.concat(newgetChanges.deleted);
                            //}
                            //if (newgetChanges.updated) {
                            //    updatedRows = newgetChanges.updated;
                            //    //$.data(dg[0], "datagrid").updatedRows.concat(newgetChanges.updated);
                            //}
                            //console.log('myLoadFilter-getChanges', newgetChanges, updatedRows, $.data(dg[0], "datagrid").updatedRows);
                        }
                    },
    ...
    $.extend($.fn.datagrid.methods, {
        enableFilter: function (jq, filters) {
            return jq.each(function () {
                ...
                //console.log(opts.onNewEndEditing);
                if (opts.oldLoadFilter) {
                    if (filters) {
