...
namespace AirOut.Web
{
    public class LuneceManager
    {
        public class IndexManager
        {
            ...
            //当前程序集
            private static System.Reflection.Assembly Assembly = AirOut.Web.Extensions.Common.Assembly;
            //索引类的 公共属性
            ...
            private void QueueToIndex(object para)
            {
                while (true)
                {
                    if (LuneceModelQueue.Count > 0)
                    {
                        ...
                        {
                            SQLDALHelper.WriteLogHelper.WriteLog("全文检索错误(线程)：" + AirOut.Web.Extensions.Common.GetExceptionMsg(ex), LuneceErrorDir, true);
                        }
                    }
                    else
                    {
            ...
            private void IndexAddDocument(IndexWriter writer, LuneceModel OLuneceModel)
            {
                ...
                {
                    SQLDALHelper.WriteLogHelper.WriteLog("创建索引出错：" + AirOut.Web.Extensions.Common.GetExceptionMsg(ex), LuneceErrorDir, true);
                    writer.Close();
                    Thread.Sleep(10);
                    ...
                    if (propInfoType == typeof(string) || propInfoType == typeof(String))
                    {
                        ...
                        if (ArrMax_StrLenAttr.Any())
                        {
                            if (ArrMax_StrLenAttr.Any())
                            {
                                ...
                                var OStrLenAttrPropertyS = ObjStrLenAttr.GetProperties();
