...
{
   public class Bms_Bill_ArQuery:QueryObject<Bms_Bill_Ar>
    {
        public Bms_Bill_ArQuery WithAnySearch(string search)
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  
                    ...
                    x.Dk_Operation_Id.Contains(search) || 
                    ...
                    x.BillEDITID.ToString().Contains(search) || 
                    x.BillEDITTS.ToString().Contains(search) || 
                    x.BillEDITWHO.ToString().Contains(search));
            return this;
        }

		public Bms_Bill_ArQuery WithPopupSearch(string search,string para="")
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  
                    x.Id.ToString().Contains(search) || 
		...
		public Bms_Bill_ArQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
				foreach (var rule in filters)
				{
					...
					if (rule.field == "Dk_Operation_Id" && !string.IsNullOrEmpty(rule.value))
					{
						 And(x => x.Dk_Operation_Id.StartsWith(rule.value));
					}
