...
{
   public class CoPoKindQuery:QueryObject<CoPoKind>
    {
        public CoPoKindQuery WithAnySearch(string search)
        {
...
x.EDITTS.ToString().Contains(search) );
		...
		public CoPoKindQuery WithPopupSearch(string search,string para="")
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  
x.ID.ToString().Contains(search) || 
...
x.EDITTS.ToString().Contains(search) );
		...
		public CoPoKindQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
