...
{
    public class CostMoneyQuery : QueryObject<CostMoney>
    {
        public CostMoneyQuery WithAnySearch(string search)
        {
            if (!string.IsNullOrEmpty(search))
                ...
                x.BSA.Contains(search) ||
                x.CustomsType.Contains(search) ||
                x.InspectMark.Contains(search) ||
                x.GetOrdMark.Contains(search) ||
        ...
        public CostMoneyQuery WithPopupSearch(string search, string para = "")
        {
            if (!string.IsNullOrEmpty(search))
                And(x =>
                x.Id.ToString().Contains(search) ||
        ...
        public CostMoneyQuery Withfilter(IEnumerable<filterRule> filters)
        {
            if (filters != null)
            {
                foreach (var rule in filters)
                {
                    ...
                    {
                        And(x => x.InspectMark.StartsWith(rule.value));
                    }
                    if (rule.field == "GetOrdMark" && !string.IsNullOrEmpty(rule.value))
                    {
                        And(x => x.GetOrdMark.StartsWith(rule.value));
                    }
                    ...
                    {
                        var decmVal = Convert.ToDecimal(rule.value);
                        And(x => x.FeeConditionVal1 == decmVal);
                    }
                    ...
                    {
                        var decmVal = Convert.ToDecimal(rule.value);
                        And(x => x.FeeConditionVal2 == decmVal);
                    }
