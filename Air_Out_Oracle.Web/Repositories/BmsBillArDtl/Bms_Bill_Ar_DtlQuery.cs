...
{
   public class Bms_Bill_Ar_DtlQuery:QueryObject<Bms_Bill_Ar_Dtl>
    {
        public Bms_Bill_Ar_DtlQuery WithAnySearch(string search)
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  
                    x.Id.ToString().Contains(search) || 
                    x.Bms_Bill_Ar_Id.ToString().Contains(search) || 
		...
		public Bms_Bill_Ar_DtlQuery WithPopupSearch(string search,string para="")
        {
            if (!string.IsNullOrEmpty(search))
                And( x =>  
                    x.Id.ToString().Contains(search) || 
		...
		public Bms_Bill_Ar_DtlQuery Withfilter(IEnumerable<filterRule> filters)
        {
           if (filters != null)
           {
				foreach (var rule in filters)
				{
					//if (rule.field == "OBms_Bill_Ar" && !string.IsNullOrEmpty(rule.value))
					//{	
					//    And(x => x.OBms_Bill_Ar == rule.value);
					//}
					...
					if (rule.field == "Bms_Bill_Ar_Id" && !string.IsNullOrEmpty(rule.value) && rule.value.IsInt())
					{
						 int val = Convert.ToInt32(rule.value);
						 And(x => x.Bms_Bill_Ar_Id == val);
					}
					...
					if (rule.field == "User_Id" && !string.IsNullOrEmpty(rule.value))
					{
						 And(x => x.User_Id.StartsWith(rule.value));
					}
					if (rule.field == "User_Name" && !string.IsNullOrEmpty(rule.value))
					{
						 And(x => x.User_Name.StartsWith(rule.value));
					}
