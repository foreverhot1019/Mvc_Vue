...
{
    public class Warehouse_receiptsController : BaseController//Controller
    {
        ...
        public Warehouse_receiptsController(IWarehouse_receiptService warehouse_receiptService, 
            IWarehouse_Cargo_SizeService warehouse_Cargo_SizeService, 
            IPictureService pictureService, 
            IPARA_PackageService para_PackageService, 
            IOPS_EntrustmentInforService oPS_EntrustmentInforService, 
            IChangeOrderHistoryService changeOrderHistoryService, 
        ...
        public ActionResult GetData(int page = 1, int rows = 10, string sort = "Id", string order = "asc", string filterRules = "")
        {
			...
			var warehouse_receipt = _warehouse_receiptService.Query(new Warehouse_receiptQuery().Withfilter(filters))
            ...
            var data = warehouse_receipt.Select(n => new
            {
                ...
                Entry_Id = n.Entry_Id, 
                Warehouse_Code = n.Warehouse_Code == null ? "" : n.Warehouse_Code, 
                Pieces_CK = n.Pieces_CK, 
                Weight_CK = n.Weight_CK, 
                Volume_CK = n.Volume_CK, 
                Packing = n.Packing == null ? "" : n.Packing,
            ...
            var pARA_Package = _unitOfWork.RepositoryAsync<PARA_Package>().Queryable().ToList();
            var bD_DEFDOC_LIST = _unitOfWork.RepositoryAsync<BD_DEFDOC_LIST>().Queryable().Where(x => x.DOCCODE == "WAREHOUSE").ToList();
            ...
            var datarows = from n in data
                       ...
                       join c in QPARA_AirLineRep on n.FLIGHT_No equals c.AirCode into c_tmp
                       from ctmp in c_tmp.DefaultIfEmpty()
                       join d in QCusBusInfo on n.Consign_Code_CK equals d.EnterpriseId into d_tmp
                       ...
                       select new
                       {
                           ...
                           Warehouse_CodeName = btmp == null ? "" : btmp.LISTNAME,
                           Pieces_CK = n.Pieces_CK,
                           Weight_CK = n.Weight_CK,
                           Volume_CK = n.Volume_CK,
                           Packing = n.Packing,
                           PackingName = ptmp == null ? "" : ptmp.PackageName,
                           CHARGE_WEIGHT_CK = n.CHARGE_WEIGHT_CK,
                           Bulk_Weight_CK = n.Bulk_Weight_CK,
                           Damaged_CK = n.Damaged_CK,
                           Damaged_Num = n.Damaged_Num,
                           ...
                           Warehouse_Remark = n.Warehouse_Remark,
                           Consign_Code_CK = n.Consign_Code_CK,
                           Consign_Code_CKNAME = dtmp == null ? n.Consign_Code_CK : dtmp.EnterpriseShortName,
                           MBL = n.MBL,
                           HBL = n.HBL,
                           //Flight_Date_Want = n.Flight_Date_Want,
                           FLIGHT_No = n.FLIGHT_No,
                           Flight_Date_Want = ctmp == null ? (n.Flight_Date_Want == null ? "" : Convert.ToDateTime(n.Flight_Date_Want).ToString("yyyy-MM-dd")) : ((n.Flight_Date_Want == null ? "" : Convert.ToDateTime(n.Flight_Date_Want).ToString("yyyy-MM-dd")) +" "+ ctmp.AirTime),
                           End_Port = n.End_Port,
                           In_Date = n.In_Date,
                           In_Time = n.In_Time,
                           Out_Date = n.Out_Date,
        ...
        public ActionResult BindingOPS(Warehouse_receipt warehousereceipt)
        {
            ...
            //    }
            //}
            string MBL = Common.RemoveNotNumber(warehousereceipt.Entry_Id);
            var entru = _oPS_EntrustmentInforService.Queryable().Where(x => (x.MBL.Equals(MBL) || x.Operation_Id.Equals(warehousereceipt.Entry_Id)) && x.Is_TG == false).ToList();
            if (entru.Count == 0 || string.IsNullOrEmpty(MBL))
        ...
        {
            var fileName = "warehouse_receipt_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
            var stream = _warehouse_receiptService.ExportExcel(filterRules, sort, order);
        ...
        /// <param name="typeName"></param>
        /// <returns></returns>
        public ActionResult PrintCoverUpWarehouseReceipt(Array filterRules = null, string typeName = "")
        {
            ...
            var rows = from n in data
                       ...
                       select new
                       {
                           ...
                           ArrWarehouse_Cargo_Size = n.ArrWarehouse_Cargo_Size == null ? null : n.ArrWarehouse_Cargo_Size.Where(x => x.Status == AirOutEnumType.UseStatusIsOrNoEnum.draft),
                           Mask = "",
                           Packing_Name = ptmp == null ? "" : ptmp.PackageCode + " " + ptmp.PackageName,
                           Remark = n.Remark,
                           Pieces_CK = n.Pieces_CK,
                           Weight_CK = n.Weight_CK,
                           Volume_CK = n.Volume_CK,
            ...
            if (rows != null)
            {
                ...
                //var Ret = WordHelper.SetWordModel_BookMarkByT<pdfJCD>(jcd, Server.MapPath("/FileModel/CoverUpWarehouseReceipt.xlsx"), ExcelFileList, Aspose.Words.SaveFormat.Doc);
                
                var outbytes = ExcelHelper.OutPutExcelByArr(ArrExcelFile.ToList(), "/FileModel/CoverUpWarehouseReceipt.xlsx", listnum, loginname, out outfile, typeName);
                var ms = new System.IO.MemoryStream(outbytes);
                Aspose.Cells.Workbook workbook = new Aspose.Cells.Workbook(ms);
                Aspose.Cells.Worksheet sheet = (Aspose.Cells.Worksheet)workbook.Worksheets[0];
        ...
        /// 导出飞力达空运放货单 PDF
        /// </summary>
        /// <param name="filterRules">条件， 仓库编号</param>
        /// <param name="typeName">单证类型</param>
        /// <returns></returns>
        public ActionResult ExportBilOfLadinPDF(Array filterRules = null, string typeName = "")
        {
            ...
            var packinglist = _unitOfWork.RepositoryAsync<PARA_Package>().Queryable().Where(x => x.Status == false).ToList();

            List<string> ArrPdfFile = new List<string>();
            List<string> ArrErrMsg = new List<string>();
            List<pdfBF> ArrExcelFile = new List<pdfBF>();

            var datarows = (from n in data
                            join x in details on n.Id equals x.Warehouse_Receipt_Id 
                            into x_tmp
                            from xtmp in x_tmp.DefaultIfEmpty()
                            join p in packinglist on n.Packing equals p.PackageCode into p_tmp
                            from ptmp in p_tmp.DefaultIfEmpty()
                            ...
                            select new
                            {
                                Entry_Id = n.Entry_Id,//进仓编号
                                Consign_Code_CK = n.Consign_Code_CK,//委托方编号
                                In_Date = n.In_Date == null ? "" : Convert.ToDateTime(n.In_Date).ToString("yyyy/MM/dd") + n.In_Time == null ? "" : n.In_Time.ToString(),//进仓时间
                                Packing = (n.Pieces_CK == null ? "" : n.Pieces_CK.ToString()) + (ptmp == null ? "" : ptmp.PackageName),//包装情况
                                DamageCondition = n.Damaged_Num == null ? "" : ("破损" + n.Damaged_Num.ToString() + "件;") + n.Dampness_Num == null ? "" : ("受潮" + n.Dampness_Num.ToString() + "件;") + n.Deformation_Num == null ? "" : ("变形" + n.Deformation_Num.ToString() + "件；"),//货损情况
                                Closure_Remark = n.Closure_Remark, // 随货文件
                                Pieces_CK = n.Pieces_CK,
                                Weight_CK = n.Weight_CK,
                                Volume_CK = n.Volume_CK,
                                Size = xtmp == null ? "" : (xtmp.CM_Length == null ? "" : xtmp.CM_Length.ToString()) + "*" + (xtmp.CM_Width == null ? "" : xtmp.CM_Width.ToString()) + "*" + (xtmp.CM_Height == null ? "" : xtmp.CM_Height.ToString()) + "*" + (xtmp.CM_Piece == null ? "" : xtmp.CM_Piece.ToString()),
            ...
            {
                    pdfBF p = new pdfBF();
                    p.Entry_Id = row.Entry_Id;
                    p.In_Date = row.In_Date;
                    p.Packing = row.Packing;
                    p.DamageCondition = row.DamageCondition;
                    p.Closure_Remark = row.Closure_Remark;
                    p.Consign_Code_CK = row.Consign_Code_CK;
                    p.Pieces_CK = row.Pieces_CK;
                    p.Weight_CK = row.Weight_CK;
                    p.Volume_CK = row.Volume_CK;
                    p.Size = row.Size;
                    p.Remark = row.Remark;
                    if (!(ArrExcelFile.Where(x => x.Entry_Id == row.Entry_Id)).Any())
                    {
                        ArrExcelFile.Add(p);
                    }
                    else
                    {
                        var item = ArrExcelFile.Where(x => x.Entry_Id == row.Entry_Id).FirstOrDefault();
                        if (item.Size.Any())
                        {
                            item.Size = item.Size + ";" + row.Size;
                        }
                        else
                        {
                            item.Size = item.Size + row.Size;
            ...
            var Ret = WordHelper.SetWordModel_BookMarkByT<pdfBF>(new pdfBF(), Server.MapPath("/FileModel/PDFBilofLading.docx"), ArrExcelFile, 0, "", Aspose.Words.SaveFormat.Doc);
            if (Ret.retResult)
            {
                ArrPdfFile.Add(Ret.MsgStr);
            ...
            var dPath = System.Configuration.ConfigurationManager.AppSettings["FileDownLoadPath"] ?? "/FileDownLoad/";
            string FName = Server.MapPath(dPath + "/pdfBF/" + fileName);
            newDoc.Save(FName, Aspose.Words.SaveFormat.Pdf);
