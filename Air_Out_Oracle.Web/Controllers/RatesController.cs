...
{
    public class RatesController : BaseController//Controller
    {
        ...
        //验证权限的名称
        private string ControllerQXName = "/Rates";

        public RatesController(IRateService rateService, IUnitOfWorkAsync unitOfWork)
            : base(false, true)
        {
        ...
        public ActionResult Index()
        {
            ...
            //var rate  = _rateService.Queryable().AsQueryable();
            //return View(rate  );
            return View();
        }
        ...
        public ActionResult GetData(int page = 1, int rows = 10, string sort = "Id", string order = "asc", string filterRules = "")
        {
            ...
            var rate = _rateService.Query(new RateQuery().Withfilter(filters)).OrderBy(n => n.OrderBy(sort, order)).SelectPage(page, rows, out totalCount);
            var data = rate.ToList();
            var datarows = data.Select(n => new
            {
                Id = n.Id,
        ...
        [HttpPost]
        public ActionResult SaveData(RateChangeViewModel rate)
        {
            if (rate.updated != null)
            {
                #region
                ...
                foreach (var updated in rate.updated)
                {
                    _rateService.Update(updated);
                }
            }
            if (rate.deleted != null)
            {
                #region
                ...
                foreach (var deleted in rate.deleted)
                {
                    _rateService.Delete(deleted);
                }
            }
            if (rate.inserted != null)
            {
                #region
                ...
                foreach (var inserted in rate.inserted)
                {
                    _rateService.Insert(inserted);
                }
            }
            ...
            try
            {
                if ((rate.updated != null && rate.updated.Any()) ||
                (rate.deleted != null && rate.deleted.Any()) ||
                (rate.inserted != null && rate.inserted.Any()))
                {
        ...
        public ActionResult Details(int? id)
        {
            ...
            Rate rate = _rateService.Find(id);
            if (rate == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Create([Bind(Include = "Id,LocalCurrency，LocalCurrCode,ForeignCurrency,ForeignCurrCode,Year,Month,RecRate,PayRate,Description,Status,OperatingPoint,ADDWHO,ADDTS,EDITWHO,EDITTS")] Rate rate)
        {
            if (ModelState.IsValid)
            {
                ...
                DisplaySuccessMessage("Has append a Rate record");
        ...
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,LocalCurrency，LocalCurrCode,ForeignCurrency,ForeignCurrCode,Year,Month,RecRate,PayRate,Description,Status,OperatingPoint,ADDWHO,ADDTS,EDITWHO,EDITTS")] Rate rate)
        {
            if (ModelState.IsValid)
            {
                ...
                DisplaySuccessMessage("Has update a Rate record");
                return RedirectToAction("Index");
            }
            if (Request.IsAjaxRequest())
            {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            DisplaySuccessMessage("Has delete a Rate record");
            return RedirectToAction("Index");
        }
        ...
        {
            var fileName = "rate_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
            var stream = _rateService.ExportExcel(filterRules, sort, order);
