...
namespace AirOut.Web.Controllers
{
    public class BD_DEFDOC_LISTsController : BaseController
    {
        ...
        public ActionResult GetData(int page = 1, int rows = 10, string sort = "ID", string order = "asc", string filterRules = "")
        {
            ...
            var bd_defdoc_list = _bD_DEFDOC_LISTService.Query(new BD_DEFDOC_LISTQuery().Withfilter(filters)).Include(b => b.BD_DEFDOC).OrderBy(n => n.OrderBy(sort, order)).SelectPage(page, rows, out totalCount);
