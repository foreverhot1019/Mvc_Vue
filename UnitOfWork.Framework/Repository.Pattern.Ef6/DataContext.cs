    ...
    public class DataContext : DbContext, IDataContextAsync
    {
        ...
        private void SyncObjectsStatePreCommit()
        {
            foreach (var dbEntityEntry in ChangeTracker.Entries())
            {
                ...
                var ObjState = ((IObjectState)dbEntityEntry.Entity).ObjectState;
                var EntyState = StateHelper.ConvertState(ObjState);

                if (EntyState != dbEntityEntry.State)
                {
                    dbEntityEntry.State = EntyState;

                    //if (ObjState == ObjectState.Unchanged && dbEntityEntry.State == EntityState.Modified)
                    //{
                    //    var OEntry = Entry(dbEntityEntry.Entity);
                    //    var Arr = dbEntityEntry.Entity.GetType().GetProperties().Select(x => x.Name);
                    //    var IsModified = false;
                    //    foreach (var name in Arr)
                    //    {
                    //        if (OEntry.Property(name).IsModified)
                    //        {
                    //            IsModified = true;
                    //            break;
                    //        }
