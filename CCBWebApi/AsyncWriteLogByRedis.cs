...
namespace CCBWebApi
{
    public class AsyncWriteLogByRedis
    {
        public class AsyncWriteLog
        {
            ...
            private readonly string RedisKeyLocalLog = WriteLogHelper.RedisKeyLocalLog;
            private readonly string RedisKeyMessageLog = WriteLogHelper.RedisKeyMessageLog;
            ...
            //启用异步写日志（数据暂存在Redis）
            public bool Async_WriteLog = false;
                        ...
                        if (OMessageToRedis != null)
                        {
                            if (OMessageToRedis.OMsg != null)
                            {
                                ...
                                if (MsgType == typeof(String))
                                    WriteLogHelper.WriteLog_Local(OMessageToRedis.OMsg.ToString(), OMessageToRedis.FolderPath, true, false, false, "", OMessageToRedis.NowDate);
                                else
                ...
                try
                {
                    if (ObjArg != null)
                    {
                        ...
                        if (OMessageToRedis != null)
                        {
                            if (OMessageToRedis.OMsg != null)
                            {
                                using (ApplicationDbContext dbContxt = new ApplicationDbContext())
                                {
                                    ...
                                    if (OMessage != null)
                                    {
                                        var OEntry = dbContxt.Entry(OMessage);
            ...
            public void ReLoadConfigSettings()
            {
                ...
                bool? _AsyncWriteLog = CacheHelper.Get_SetBoolConfAppSettings("Global", "AsyncWriteLog");
            ...
            {
                WriteLogHelper.WriteLog_Local(ErrMsgStr, FolderPath, DayFilePathName, false, HourFileName, FileName);
            }
        }
    }
}